...
const Projects = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        // For now, let's use the demo data until we implement the full projects functionality
        import("@/data/demoData").then(({ demoProjects }) => {
          setProjects(demoProjects as Project[]);
          
          // Extract all unique skills from projects
          const skills = Array.from(
            new Set(demoProjects.flatMap((project) => project.skills))
          ).sort();
        ...
        toast({
          ...
          description: "Не удалось загрузить данные проектов",
          variant: "destructive",
        });
        setIsLoading(false);
      }
  ...
  const filteredProjects = projects.filter((project) => {
    ...
    const matchesCurrency =
      !selectedCurrency || selectedCurrency === 'all' || project.currency === selectedCurrency;
